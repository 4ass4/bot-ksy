...
export class UnifiedHandlers {
    ...
    private officialChannelService: OfficialChannelService;
    private botSettingsService: BotSettingsService;
    private referralService: ReferralService;
    private analyticsService: AnalyticsService;
    private socialService: SocialService;
    private socialAccountService: SocialAccountService;
    private adminStates: { [key: number]: AdminState } = {};
    private adminIds: number[];

    // Универсальный метод для отправки сообщений с fallback
    private async sendMessage(ctx: Context, text: string, options: any = {}) {
        try {
...
Выберите действие:`;
    ...
    async handleUserCallback(ctx: Context, data: string) {
        if (!ctx.from) return;
    ...
    async handleAdminCallback(ctx: Context, data: string) {
        if (!ctx.from) return;
        switch (data) {
            case 'admin_raffles':
    ...
    async handleAdminText(ctx: Context, text: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleAdminMedia(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    // ===== АДМИНСКИЕ МЕТОДЫ =====
    async showAdminMain(ctx: Context) {
        const adminText = `🔧 *Админ-панель*
...
Выберите действие:`;
    ...
    async showAdminRaffles(ctx: Context) {
...
Выберите действие:`;
    ...
    async showAdminChannels(ctx: Context) {
...
Выберите действие:`;
    ...
    async showAdminMailings(ctx: Context) {
...
Выберите действие:`;
    ...
    async showAdminStats(ctx: Context) {
...
• Всего: ${stats.totalMailings}`;
    ...
    async showBotSettings(ctx: Context) {
...
💡 Выберите действие для изменения настроек:`;
    ...
    async startCreateRaffle(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleCreateRaffleTitle(ctx: Context, text: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleCreateRafflePrize(ctx: Context, text: string) {
        const userId = ctx.from.id;
    ...
    async handleCreateRaffleWinners(ctx: Context, text: string) {
    ...
    async handleCreateRaffleDuration(ctx: Context, text: string) {
    ...
    async handleAddRafflePhoto(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleAddRaffleVideo(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleSkipRaffleMedia(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleRafflePhoto(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleRaffleVideo(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async showChannelSelection(ctx: Context) {
    ...
    async updateChannelSelectionMessage(ctx: Context) {
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
        ...
        socialAccounts.forEach((account, index) => {
            ...
            if ((channels.length + index + 1) % 2 === 0) keyboard.row();
        });
    ...
    async handleSelectChannel(ctx: Context, channelId: number) {
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
        const selectedChannels = state.tempData.selectedChannels || [];
    ...
    async handleSelectSocialAccount(ctx: Context, socialAccountId: number) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
        const selectedSocialAccounts = state.tempData.selectedSocialAccounts || [];
    ...
    async handleCreateRaffleFinal(ctx: Context) {
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
        ...
        const raffle = await this.raffleService.createRaffle(
            tempData.prize, 
            tempData.winnersCount, 
            endDate, 
            tempData.selectedChannels || [],
            tempData.photoFileId,
            tempData.videoFileId,
            tempData.referralRequirement || false,
            tempData.minReferrals || 0,
            tempData.maxReferrals || 0,
            tempData.referralBonus
        );
        ...
        for (const socialAccountId of selectedSocialAccounts) {
            await this.socialAccountService.addSocialRequirementToRaffle(
                raffle.id,
                socialAccountId,
                'FOLLOW'
            );
    ...
    async showActiveRafflesAdmin(ctx: Context) {
    ...
    async showChannelsList(ctx: Context) {
        const channels = await this.channelService.getAllChannels();
        if (channels.length === 0) {
    ...
    async startAddChannel(ctx: Context) {
        const userId = ctx.from.id;
        this.adminStates[userId] = {
            step: 'add_channel',
            tempData: {}
    ...
    async handleAddChannel(ctx: Context, text: string) {
        console.log(`➕ Обработка добавления канала`);
    ...
    async startEditRaffle(ctx: Context, raffleId: number) {
...
💡 Выберите что хотите изменить:`;
    ...
    async publishRaffle(ctx: Context, raffleId: number) {
...
💡 Теперь пользователи могут участвовать в розыгрыше!`;
    ...
    async startCreateMailing(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        this.adminStates[userId] = {
    ...
    async handleSelectMailingType(ctx: Context, mailingType: MailingType) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleCreateMailingText(ctx: Context, text: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    async handleSelectMailingSchedule(ctx: Context, scheduleType: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    async handleCreateMailingCustomTime(ctx: Context, timeText: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleAddMailingPhoto(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    async handleAddMailingVideo(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    async handleSkipMailingMedia(ctx: Context) {
        await this.showMailingScheduleSelection(ctx);
    }

    async showMailingScheduleSelection(ctx: Context) {
        try {
    ...
    async createMailingFinal(ctx: Context, scheduleTime?: Date) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
        
        try {
            const mailing = await this.mailingService.createMailing(
                state.tempData.mailingType,
                state.tempData.messageText,
                state.tempData.targetRaffleId,
                state.tempData.photoFileId, // photoFileId
    ...
    async showMailingsList(ctx: Context) {
    ...
    async showChannelsToDelete(ctx: Context) {
        const channels = await this.channelService.getAllChannels();
        if (channels.length === 0) {
    ...
    async showRafflesToDelete(ctx: Context) {
    ...
    async showActiveRafflesToDelete(ctx: Context) {
    ...
    async showFinishedRafflesToDelete(ctx: Context) {
    ...
    async handleDeleteChannel(ctx: Context, channelId: number) {
        try {
    ...
    async handleDeleteRaffle(ctx: Context, raffleId: number) {
        try {
            const raffle = await this.raffleService.getRaffleById(raffleId);
            if (!raffle) {
    ...
    async handleParticipation(ctx: Context, raffleId: number) {
...
🍀 Удачи! Надеемся, что именно вы станете победителем!`;
    ...
    async showSubscriptionChannels(ctx: Context, raffle: any, channels: any[]) {
        const channelText = channels.length === 1 ? 'канал' : 'каналы';
        let text = `📺 *Для участия в розыгрыше подпишитесь на ${channelText}:*
...
⏰ Завершение: ${raffle.end_date.toLocaleDateString('ru-RU')}`;
    ...
    async handleSubscribeToChannel(ctx: Context, channelId: number) {
...
3️⃣ Вернитесь в бот и нажмите "🔍 Проверить подписку"`, {
    ...
    async checkSubscriptionAndParticipate(ctx: Context, raffleId: number) {
...
🍀 Удачи! Надеемся, что именно вы станете победителем!`;
    ...
    async checkSubscriptions(ctx: Context, raffleId: number) {
        await this.handleParticipation(ctx, raffleId);
    }

    async checkSocialSubscriptions(ctx: Context, raffleId: number) {
        ...
        for (const requirement of socialAccounts) {
            try {
                ...
                const subscribed = await this.socialAccountService.checkSocialSubscription(
                    requirement.social_account.platform,
                    requirement.social_account.username,
                    user.telegram_id
                );
                subscriptionResults.push({ account: requirement.social_account, subscribed });
                if (!subscribed) allSubscribed = false;
            } catch (error) {
...
📊 *Результаты проверки:*\n`;
    ...
    async checkCombinedSubscriptions(ctx: Context, raffleId: number) {
        ...
        for (const channel of channels) {
            try {
                ...
                if (!subscribed) allChannelsSubscribed = false;
            } catch (error) {
        ...
        for (const requirement of socialAccounts) {
            try {
                ...
                if (!subscribed) allSocialSubscribed = false;
            } catch (error) {
                console.log(`❌ Ошибка проверки подписки на социальный аккаунт ${requirement.social_account.display_name}:`, error);
...
📊 *Результаты проверки:*\n`;
    ...
    async showUserRaffles(ctx: Context) {
...
4. Нажмите "Участвовать"`;
    ...
    async showActiveRaffles(ctx: Context) {
...
🎉 Следите за нашими каналами - новые розыгрыши появляются регулярно!`;
    ...
    async showRaffleDetails(ctx: Context, raffleId: number) {
...
💡 *Важно:* Для участия в розыгрыше необходимо быть подписанным на все указанные каналы-партнеры.`;
    ...
    async showOfficialChannel(ctx: Context) {
...
💡 Выберите действие из меню ниже:`;
    ...
    async handleAdminPanelAccess(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
...
💡 Если вы считаете, что это ошибка, обратитесь к администратору.`;
    ...
    async showAdminOfficialChannel(ctx: Context) {
...
Выберите действие:`;
    ...
    async startAddOfficialChannel(ctx: Context) {
        const userId = ctx.from.id;
...
💡 Это будет основной канал, который будет отображаться пользователям в меню "Наш официальный канал"`;
    ...
    async showOfficialChannelInfo(ctx: Context) {
        const officialChannel = await this.officialChannelService.getOfficialChannel();
        if (!officialChannel) {
...
💡 Этот канал отображается пользователям в меню "Наш официальный канал"`;
    ...
    async deleteOfficialChannel(ctx: Context) {
...
💡 Добавьте новый официальный канал, если необходимо.`;
    ...
    async handleAddOfficialChannel(ctx: Context, text: string) {
        console.log('➕ Обработка добавления официального канала');
...
💡 Теперь пользователи смогут подписываться на ваш официальный канал через меню "Наш официальный канал"`;
    ...
    async startEditOfficialChannelDescription(ctx: Context) {
        const officialChannel = await this.officialChannelService.getOfficialChannel();
        if (!officialChannel) {
            const text = `❌ *Официальный канал не настроен*
...
💡 Описание будет отображаться пользователям при просмотре официального канала.`;
    ...
    async handleEditOfficialChannelDescription(ctx: Context, text: string) {
        console.log('✏️ Обработка редактирования описания официального канала');
...
💡 Теперь пользователи увидят новое описание при просмотре официального канала.`;
    ...
    async showMailingsToDelete(ctx: Context) {
        const mailings = await this.mailingService.getAllMailings();
        if (mailings.length === 0) {
            await ctx.editMessageText('📭 Рассылок не найдено.', {
                reply_markup: Keyboards.backToAdmin()
    ...
    async handleDeleteMailing(ctx: Context, mailingId: number) {
        try {
    ...
    async handleMailingPhoto(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    async handleMailingVideo(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
        const state = this.adminStates[userId];
    ...
    async startEditWelcomeMessage(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async startAddCoverPhoto(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async removeCoverPhoto(ctx: Context) {
        try {
    ...
    async handleEditWelcomeMessageText(ctx: Context, text: string) {
        try {
    ...
    async handleCoverPhoto(ctx: Context) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleRaffleTypeSelection(ctx: Context, type: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleReferralRequirementSelection(ctx: Context, requirement: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async handleCreateRaffleReferralCount(ctx: Context, text: string) {
        if (!ctx.from) return;
        const userId = ctx.from.id;
    ...
    async showReferralSystem(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showReferralSystemWithContext(ctx: Context, raffleId: number) {
        if (!ctx.from) return;
    ...
    async showReferralLink(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showReferralStats(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showReferralList(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showReferralTop(ctx: Context) {
    ...
    async copyReferralLink(ctx: Context) {
        await ctx.answerCallbackQuery('📋 Ссылка скопирована в буфер обмена!');
    }

    async shareReferralLink(ctx: Context) {
    ...
    async checkReferralRequirements(ctx: Context, raffleId: number) {
        const user = await this.userService.getUserByTelegramId(ctx.from.id);
        if (!user) return;
...
\`https://t.me/${ctx.me.username}?start=REF${user.id}\``;
    ...
    private async getAutoEnrolledRaffles(userId: number): Promise<any[]> {
        try {
            // Получаем все розыгрыши пользователя
            const userRaffles = await this.raffleService.getUserParticipatedRaffles(userId);
            
            // Фильтруем только активные розыгрыши
            const autoEnrolledRaffles = userRaffles.filter(raffle => 
    ...
    async showReferralLinkWithContext(ctx: Context, raffleId: number) {
        if (!ctx.from) return;
    ...
    async showReferralStatsWithContext(ctx: Context, raffleId: number) {
        if (!ctx.from) return;
    ...
    async showReferralListWithContext(ctx: Context, raffleId: number) {
        if (!ctx.from) return;
    ...
    async showReferralTopWithContext(ctx: Context, raffleId: number) {
        // Показываем уведомление о том, что топ обновлен
        await ctx.answerCallbackQuery('🏆 Топ рефералов обновлен');
    ...
    async showAdminSocial(ctx: Context) {
        if (!ctx.from) return;
    ...
    async startAddSocialAccount(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showSocialAccountsList(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showSocialAccountsStats(ctx: Context) {
        if (!ctx.from) return;
    ...
    async showSocialAccounts(ctx: Context, raffle: any, socialAccounts: any[]) {
...
📱 Социальные аккаунты:\n`;
    ...
    async showCombinedRequirements(ctx: Context, raffle: any, channels: any[], socialAccounts: any[]) {
...
📺 Каналы для подписки:\n`;
    ...
    async handleSocialPlatformSelection(ctx: Context, platform: string) {
        if (!ctx.from) return;
    ...
    async handleSocialUsernameInput(ctx: Context, username: string) {
        if (!ctx.from) return;
    ...
    async handleSocialDisplayNameInput(ctx: Context, displayName: string) {
        if (!ctx.from) return;
    ...
    async handleSocialProfileUrlInput(ctx: Context, profileUrl: string) {
        if (!ctx.from) return;
    ...
    async handleSocialFollowerCountInput(ctx: Context, followerCountText: string) {
        if (!ctx.from) return;
    ...
    async handleSocialVerifiedSelection(ctx: Context, isVerified: boolean) {
        if (!ctx.from) return;
        ...
        try {
            ...
            const socialAccount = await this.socialAccountService.createSocialAccount(
                state.tempData.platform,
                state.tempData.username,
                state.tempData.display_name,
                state.tempData.profile_url,
                state.tempData.follower_count,
                isVerified
            );
