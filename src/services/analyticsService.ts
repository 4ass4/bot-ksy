...
import { Analytics, AnalyticsEventType } from '../types/index.js';
...
export class AnalyticsService {
  ...
  async getUserStats(): Promise<{
    ...
    const activeToday = await this.db.get<any>(`
  ...
  async getRaffleStats(): Promise<{
    ...
    const participants = await this.db.get<any>(`
      SELECT COUNT(*) as total_participants
      FROM participants
    `);

    const winners = await this.db.get<any>(`
      SELECT COUNT(*) as total_winners
      FROM winners
    `);

    const avgParticipants = await this.db.get<any>(`
  ...
  async getChannelStats(): Promise<{
    ...
    topChannels: Array<{ name: string; subscriptions: number }>;
  }> {
    const stats = await this.db.get<any>(`
    ...
    const subscriptions = await this.db.get<any>(`
      SELECT COUNT(*) as total_subscriptions
      FROM analytics
      WHERE event_type = ?
    `, [AnalyticsEventType.USER_SUBSCRIBED_CHANNEL]);

    const avgSubscriptions = await this.db.get<any>(`
    ...
    const topChannels = await this.db.all<any>(`
  ...
  async getReferralStats(): Promise<{
    ...
    const topReferrers = await this.db.all<any>(`
  ...
  async getTimeStats(days: number = 30): Promise<{
    registrations: Array<{ date: string; count: number }>;
    raffleJoins: Array<{ date: string; count: number }>;
    channelSubscriptions: Array<{ date: string; count: number }>;
  }> {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);

    const registrations = await this.db.all<any>(`
    ...
    const raffleJoins = await this.db.all<any>(`
      SELECT DATE(created_at) as date, COUNT(*) as count
      FROM analytics
      WHERE event_type = ? AND created_at >= ?
      GROUP BY DATE(created_at)
    ...
    const channelSubscriptions = await this.db.all<any>(`
      SELECT DATE(created_at) as date, COUNT(*) as count
      FROM analytics
      WHERE event_type = ? AND created_at >= ?
      GROUP BY DATE(created_at)
